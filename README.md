...
- 🔤 **Text Manipulation Tools** - Quick access to romanization, case conversion, and text formatting
  ...
  - Translation tools
- ⚙️ **Configurable SRT Export** - Fine-tune subtitle generation with customizable settings:
  - Maximum characters per line
  - Minimum subtitle duration
  - Gap frames between subtitles
...
1. **Clone the repository**
   ```bash
   git clone https://github.com/SagarParmar90/Subtitle-creator.git
   cd Subtitle-creator
   ```
...
### Option 1: Generate Subtitles from Audio

1. **Upload Audio** - Drop your MP3 or WAV file into the upload zone
...
### Option 2: Import & Edit Existing SRT Files

1. **Upload SRT** - Drop your existing .srt subtitle file into the upload zone
2. **Edit** - Use all the powerful editing tools:
   - Modify text and timestamps
   - Apply text transformations
   - Search and verify content
   - Use romanization for non-English text
3. **Export** - Download in any format:
   - Re-export as improved .srt with custom settings
   - Convert to .txt, .csv, or .json
   - Generate prtranscript for AI workflows
...
**Note:** Audio playback is only available when you upload an audio file (MP3/WAV). If you import an SRT file without audio, you can still edit all text and timestamps - just without audio preview.
...
Customize your subtitle output to match platform requirements:

**Max Characters per Line**
- Default: 42 characters
- Adjustable for different platforms:
  - YouTube: 40-50 characters
  - Instagram/TikTok: 20-30 characters (mobile-friendly)
  - TV/Cinema: 35-42 characters
  - Custom: Set any value for specific needs

**Minimum Subtitle Duration**
- Default: 1 second
- Ensures subtitles display long enough to be read
- Adjustable based on reading speed preferences

**Gap Frames Between Subtitles**
- Controls spacing between consecutive subtitle blocks
- Prevents subtitles from appearing too close together
- Improves visual clarity and readability

**Line Formatting**
- **Single-line**: All text on one line (compact, good for short phrases)
- **Double-line**: Text split across two lines (better readability for longer phrases)

These settings can be configured before export, allowing you to generate platform-specific subtitle files from the same transcription.
...
- **Character limits** (configurable, prevents text overflow)
- **Reading speed** (optimal duration for viewer comprehension)

These parameters work in combination with your export settings to create professional-quality subtitles automatically.
...
### How do I configure SRT export settings?
Before exporting as .srt, you can customize:
- **Max characters per line**: Adjust for different platforms (YouTube needs ~40-50, Instagram/TikTok ~20-30)
- **Minimum duration**: Set how long each subtitle should display
- **Gap frames**: Control spacing between subtitle blocks
- **Line format**: Choose single-line (compact) or double-line (more readable)

These settings help you create platform-specific subtitles optimized for YouTube, Instagram, TikTok, or professional video editing.

### What settings should I use for different platforms?
**YouTube/Vimeo:**
- Max chars: 40-50
- Double-line format
- Standard gap frames
...
- Max chars: 20-30
- Single-line format (less screen space)
- Shorter minimum duration

**Professional Video Editing:**
- Max chars: 35-42
- Double-line format
- Adjust based on project specs
...
### Upload Not Working
- Check file format (MP3, WAV, or SRT supported)
- Try a smaller file size
- Clear browser cache and try again
- For SRT files, ensure they're properly formatted
...
### SRT File Not Parsing Correctly
- Ensure your SRT file follows standard formatting
- Check that timestamps are in correct format (HH:MM:SS,mmm)
...
- Use the configurable SRT export settings to match your platform
- Try different max characters per line (YouTube: 40-50, Instagram: 20-30)
- Adjust line formatting (single vs double-line)
- Export format is optimized for professional video editors
- The grouping algorithm adapts automatically with your settings

### Subtitles Too Long/Short on Screen
- Adjust **minimum subtitle duration** in export settings
- Increase **gap frames** for more spacing between subtitles
- Change **max characters per line** to fit more/less text
- Try different line formatting options

### Need More Help?
1. Check the [GitHub Issues](https://github.com/SagarParmar90/Subtitle-creator/issues)
...
**Major Update - SRT Import & Enhanced Editor**
- ✅ Audio file upload (MP3/WAV)
- ✅ **SRT file import** - Edit existing subtitle files
- ✅ **Format conversion** - Import SRT, export to any format
- ✅ 15+ language support
- ✅ Word-level timestamp generation
- ✅ Automatic subtitle sanitization (timestamp validation & correction)
- ✅ Advanced subtitle editor with integrated tools
- ✅ Conditional audio playback (available with audio files)
- ✅ Search Widget with customizable URL templates
- ✅ Text Tools Widget (romanization, case conversion, spell check, translation)
- ✅ **Configurable SRT Export Settings**
  - Max characters per line (platform-specific)
  - Minimum subtitle duration
  - Gap frames between subtitles
  - Single/double-line formatting
